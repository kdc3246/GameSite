...
function showCountDown(){
    ...
    countDown.textContent=gameDatas.count_down;
}
...
function clear(){
    ...
    numberWrapper.removeChild(numberWrapper.lastElementChild);
}
...
//실패인 경우 처리할 일
function processFail(target) {
    //1. 축하박스 등장 처리
    const $finish = document.getElementById('fail');
    $finish.classList.add('show');
    clear();
}
//정답을 검증하는 함수
function checkAnswer(target) {
    gameDatas.count_down-=1;
    showCountDown();
    //객체 디스트럭쳐링
    const {secret, answer} = gameDatas;

    if(!gameDatas.count_down){
        processFail(target);
    }
